...
module.exports = async function handler(req, res) {
    ...
    const requestBody = JSON.stringify({
        ...
        generationConfig: {
            temperature: 0.85,
            maxOutputTokens: 8192,  // 4096 → 8192 (완전한 동화 생성)
            topP: 0.85,
            candidateCount: 1,
            stopSequences: []
        }
...
function buildPrompt(keywords, mood) {
    ...
    return [
        ...
        '- 이야기 길이는 공백 제외 약 800-1000자 정도로 작성합니다.',
        '- 서론-전개-결말 구조를 갖추고 인물의 감정과 배경을 구체적으로 묘사합니다.',
        '- 문단을 3-4개로 나누고 각 문단은 줄바꿈으로 구분합니다.',
        '- 반드시 이야기를 완결하고 마지막 문단에서 어린 독자에게 여운이 남는 마무리 문장을 제공합니다.',
        '- 절대 중간에 끊기지 않고 완전한 이야기로 끝맺음하세요.',
        '',
        '키워드 목록:',
        keywordList,
        '',
        '위 조건을 모두 충족하는 완결된 한국어 동화를 작성하세요.'
    ].join('\n');
}
